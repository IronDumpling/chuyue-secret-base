...
- **Posts and projects** are written once per language: `her-review.en.mdx` and `her-review.zh.mdx` in the same folder, same slug. Every file must carry its language in the name, and its text must really be in that language: `npm run sync:content` (which `dev` and `build` run first) fails when a file has no `.en`/`.zh`, when an `.en` file is more than about 5% Chinese or a `.zh` file less than about 20%, when the title or description is in the wrong language, or when the two versions disagree on the shared fields (date, rating, tags, images, github, demo). So a post cannot hold both languages in one file. If a language is missing, the page shows the other language with a notice, and its canonical link points at the real one.
